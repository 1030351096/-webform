...
$("#btn_ImgUpStart").click(function () {
    ...
    for (var i = 0; i < $(".lookimg").length; i++) {
        ...
        $.ajax({
            type: "POST",
            url: "./ImgUpLoad.ashx",
            data: { 'imgBase': IMG_BASE, 'imgFormat': IMG_FOMATE, 'lookIndex': NOWLOOK.index, 'id': id, 'texts': arr },//图片base64编码，图片格式（当前仅支持jpg,png,jpeg三种），图片对象索引
            dataType: "json",
            async: false,
            success: function (data) {
                if (data.isok == "1") {
                    ...
                    $(".lookimg").eq([data.ind]).children(".lookimg_progress").eq(0).children("div").eq(0).animate({ width: "100%" }, UPTIME, function () {
                        ...
                        window.location.href = "commodity.aspx";
                    });
                }
                else {//图片未上传成功回调
