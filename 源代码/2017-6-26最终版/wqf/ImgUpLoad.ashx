...
public class ImgUpLoad : IHttpHandler
{
    ...
    public void ProcessRequest(HttpContext context)
    {
        ...
        string text = context.Request["texts"];
        ...
        string serverPath = "../img/" + DateTime.Now.ToString("yyyyMMddhhmmssms") + ran.Next() + "." + imgFomate;

        string imgurl = serverPath + ",";
        using (DataClassesDataContext con = new DataClassesDataContext())
        {
            Commoimg coimg = con.Commoimg.Where(r => r.Commoimg1 == id).FirstOrDefault();
            if (coimg != null)
            {
                con.ExecuteCommand("update Commoimg set Commoimg2=Commoimg2+'" + imgurl + "',Commo_details='" + text + "' where Commoimg1='" + id + "' ");
